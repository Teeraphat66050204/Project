  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-6">
      <div className="mx-auto max-w-md card-premium p-6">
        ...
        <p className="mt-2 text-sm text-[rgba(249,250,251,0.70)]">Join ChaoLoey and track every booking in one place.</p>
        <form onSubmit={onSubmit} className="mt-5 space-y-4">
          ...
          <div>
            ...
            <PasswordField id="password" name="password" required minLength={6} placeholder="At least 6 characters" />
          </div>
          <div>
            <label htmlFor="confirmPassword" className="mb-1 block text-sm font-semibold">Confirm password</label>
            <PasswordField id="confirmPassword" name="confirmPassword" required minLength={6} placeholder="Re-enter password" />
          </div>
          {error ? <p className="status-banner">{error}</p> : null}
          <button type="submit" disabled={loading} className="btn-base btn-primary w-full px-4 py-3">{loading ? "Creating account..." : "Create account"}</button>
        </form>
